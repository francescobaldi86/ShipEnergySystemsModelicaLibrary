...
condition to calculate the length of each section.
 ...
 <p><big>The assumptions for this model are:
  ...
  model HEX_1f
    "Counter current simple HEX model. No fluid undergo any phase change. The primary fluid is a proper Modelica fluid, and should be in liquid state (hence the ''secondary fluid - to - liquid'' denomination)"
  ...
  /***************** PORTS ******************/
    ...
    ShipEnergySystems.Other.FlangeCdot_B OutFlow_sf
      annotation (Placement(transformation(extent={{-110,50},{-90,70}})));
    ShipEnergySystems.Other.FlangeCdot_A InFlow_sf
  ...
  /*********** GENERAL VARIABLES AND PARAMETER ***********/
  parameter Modelica.SIunits.Area A_ht
      "Heat transfer area";
  ...
  /*************** Wall between the fluids *************************/
  parameter Modelica.SIunits.Mass M_wall
  ...
  //parameter Modelica.SIunits.MassFlowRate Mdotnom
  //    "Nominal fluid flow rate of the primary fluid";
  Modelica.SIunits.MassFlowRate mdot_pf_in
      "Mass flow at the inlet of the heat exchanger";
  Modelica.SIunits.MassFlowRate mdot_pf_out
  ...
  Modelica.SIunits.Temperature T_sf_in
      "Tempearture at the inlet of the secondary fluid";
  Modelica.SIunits.Temperature T_sf
      "Mean Temperatue of the subcooled region secondary fluid";
  Modelica.SIunits.Temperature T_sf_out
      "Secondary fluid temperature at the exit of the heat exchanger";
  
  /*************** HEAT TRANSFER PARAMETER ****************/
  parameter Modelica.SIunits.CoefficientOfHeatTransfer U_pf
      "Heat transfer coefficient of the primary fluid";
  ...
  Modelica.SIunits.SpecificHeatCapacity cp_pf
      "Specific heat capacity of the secondary fluid at the inlet";
  
  
  /************** Enthalpies ************************/
  Modelica.SIunits.SpecificEnthalpy h_pf "Mean Enthalpy of the primary fluid within the heat exchanger";
  Modelica.SIunits.SpecificEnthalpy h_pf_in
      "Enthalpy at the inlet of the heat exchanger of the primary fluid";
  Modelica.SIunits.SpecificEnthalpy h_pf_out
      "Enthalpy at the outlet of the heat exchanger of the primary fluid";
  
  /***** Enthalpy&Density****/
  
  /******************* Density derivatives ********************/
  Modelica.SIunits.DerDensityByEnthalpy drdh_pf
      "Density derivative with respect to enthalpy at constant pressure for the primary fluid";
  Modelica.SIunits.DerDensityByPressure drdp_pf
      "Density derivative with respect to pressure at constant enthalpy for the primary fluid";
  ...
  Q_pf = cp_pf * (mdot_pf_in + mdot_pf_out)/2 * epsilon_pf * (T_w_sf_in - T_pf_in)
      "Heat from the wall to the primary fluid";
...
Modelica.SIunits.MassFlowRate mdot_pf_out
    "Mass flow at the outlet of the heat exchanger";
Modelica.SIunits.MassFlowRate mdot_pf_sl2ev
    "Mass flow of the primary fluid at the interface between the two regions";
Modelica.SIunits.MassFlowRate mdot_sf
    "Mass flow of the secondary fluid of the heat exchanger";
...
/*************** HEAT TRANSFER PARAMETER ****************/
parameter Modelica.SIunits.CoefficientOfHeatTransfer U_pf_sl
    "Heat transfer coefficient of the primary fluid, subcooled region";
parameter Modelica.SIunits.CoefficientOfHeatTransfer U_pf_ev
    "Heat transfer coefficient of the primary fluid, evaporation";
parameter Modelica.SIunits.CoefficientOfHeatTransfer U_sf
    "Secondary fluid Heat transfer coefficient sub-cooled side";
parameter Modelica.SIunits.QualityFactor eta_fin = 1 "Fin efficiency";
...
Modelica.SIunits.SpecificHeatCapacity cp_sf
    "Specific heat capacity of the secondary fluid at the inlet";
Modelica.SIunits.SpecificHeatCapacity cp_pf_sl
    "Specific heat capacity of the primary fluid in the subcooled liquid region";
...
/******************* Density derivatives ********************/
Modelica.SIunits.DerDensityByEnthalpy drdh_pf_sl
    "Density derivative with respect to enthalpy at constant pressure for the primary fluid in the subcooled liquid region";
Modelica.SIunits.DerDensityByPressure drdp_pf_sl
    "Density derivative with respect to pressure at constant enthalpy for the primary fluid in the subcooled liquid region";
Modelica.SIunits.DerDensityByPressure drdp_pf_ls
    "Density derivative with respect to pressure at constant enthalpy for the primary fluid in along the saturated liquid curve";
Modelica.SIunits.DerDensityByPressure drdp_pf_vs
    "Density derivative with respect to pressure at constant enthalpy for the primary fluid along the saturated vapor curve";
Modelica.SIunits.DerEnthalpyByPressure dhdp_pf_ls
    "Enthalpy derivative with respect to pressure for the primary fluid in along the saturated liquid curve";
Modelica.SIunits.DerEnthalpyByPressure dhdp_pf_vs
    "Enthalpy derivative with respect to pressure for the primary fluid along the saturated vapor curve";
...
/************ MassBalance Primary fluid side ****************/
// Subcooled liquid region //
pi*D^2/4 * (L_sl * (drdp_pf_sl + drdh_pf_sl * dhdp_pf_ls) * der(p) + (rho_pf_sl - rho_pf_ls) * der(L_sl) + 0.5 * L_sl * drdh_pf_sl * der(h_pf_in)) = mdot_pf_in - mdot_pf_sl2ev; // Updated
// Evaporation region //
pi*D^2/4 * ((rho_pf_ls - rho_pf_vs) * der(L_sl) + (1-void) * (rho_pf_ls - rho_pf_vs) * der(L_ev) + L_ev * (void*drdp_pf_vs + (1-void)*drdp_pf_ls) * der(p)) = mdot_pf_sl2ev - mdot_pf_out ; // Updated

/********** EnergyBalance Primary fluid side ******************/
// Subcooled liquid region //
0.5 * pi*D^2/4 * ((rho_pf_sl * (h_pf_in + h_pf_ls) - 2*rho_pf_sl*h_pf_ls) * der(L_sl) + (rho_pf_sl * L_sl + drdh_pf_sl) * der(h_pf_in) + L_sl * (rho_pf_sl * dhdp_pf_ls + (h_pf_in + h_pf_ls) * (drdp_pf_sl + 0.5*drdh_pf_sl * dhdp_pf_ls - 2)))* der(p) = mdot_pf_in * h_pf_in - mdot_pf_sl2ev * h_pf_ls + Q_pf_sl; // Updated
...
Q_SH = cp_SH* (M_dot_B +M_dot_ex)/2  * (TwSH - T_TP) //* epsilon_SH_pf
